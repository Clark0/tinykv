...
func (rn *RawNode) Ready() Ready {
	...
	if hardSt := rn.Raft.hardState(); !isHardStateEqual(rn.prevHardSt, hardSt) {
...
func (rn *RawNode) HasReady() bool {
	...
	if len(r.msgs) > 0 || len(r.RaftLog.unstableEntries()) > 0 || len(rn.Raft.RaftLog.nextEnts()) > 0 {
		return true
	}
	if !IsEmptySnap(r.RaftLog.pendingSnapshot) {
		return true
...
func (rn *RawNode) Advance(rd Ready) {
	...
	// update raft applied index
	if len(rd.CommittedEntries) > 0 {
		rn.Raft.RaftLog.applied = rd.CommittedEntries[len(rd.CommittedEntries)-1].Index
	}
	// update raft stabled index
	if len(rd.Entries) > 0 {
